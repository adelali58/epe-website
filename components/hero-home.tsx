...
export default function HeroHome() {
  const handleEmergencyMaintenance = () => {
    // 1. التأكد من دعم المتصفح لتحديد الموقع (GPS)
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          // 2. سحب خطوط الطول والعرض وتجهيز لينك جوجل ماب
          const lat = position.coords.latitude;
          const lng = position.coords.longitude;
          const googleMapsUrl = `https://www.google.com/maps?q=${lat},${lng}`;

          // 3. طلب اسم العميل ورقمه (نوافذ منبثقة سريعة)
          ...
          // 4. تجهيز الرسالة وفتح الواتساب بالرقم الجديد
